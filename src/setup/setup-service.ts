...
type TargetConfig = Readonly<{
  ...
  // Optional lifecycle-hook extension (currently only the Copilot CLI target).
  extensionFilePath?: (home: string) => string;
}>;
...
const TARGET_CONFIG: Record<SetupTarget, TargetConfig> = {
  ...
  copilot: {
    ...
    extensionFilePath: (home) =>
      join(
        home,
        ".copilot",
        "extensions",
        "cairn-session-primer",
        "extension.mjs",
      ),
  },
};
...
function copilotExtensionContent(): string {
  return `// Cairn session primer for the GitHub Copilot CLI.
// Generated by \`cairn --setup copilot\`; regenerate with the same command.
//
// On every session start this injects the current project's Cairn work and
// memory context so the agent is primed automatically, instead of relying on it
// to remember to run Cairn. It is a safe no-op outside Cairn projects.
//
// Cairn is expected on PATH (e.g. installed via Homebrew). Set the CAIRN_BIN
// environment variable to override the binary location.
...
// Candidate cairn binaries, tried in order: CAIRN_BIN override, then "cairn"
// on PATH (the Homebrew install location).
const CANDIDATES = [process.env.CAIRN_BIN, "cairn"].filter(Boolean);

async function cairn(bin, args, cwd) {
...
async function resolveProject(cwd) {
  for (const bin of CANDIDATES) {
    try {
      const status = JSON.parse(await cairn(bin, ["status", "--json"], cwd));
      if (status && status.projectId) {
        return { bin, status };
      }
    } catch {
      // Not this binary, or not a Cairn project via it; try the next candidate.
    }
  }
  return undefined;
}

async function safe(bin, args, cwd) {
  try {
    return await cairn(bin, args, cwd);
  } catch {
    return undefined;
  }
}
...
async function buildContext(cwd) {
  ...
  const { bin, status } = resolved;

  const [ready, memory] = await Promise.all([
    safe(bin, ["work", "ready", "--explain", "--json"], cwd),
    safe(bin, ["memory", "context", "--json"], cwd),
  ]);

  const lines = [
    "# Cairn is active for this project (MANDATORY protocol)",
    "",
    "Cairn is the source of truth for work tracking, memory, and project context",
    "in this repository. Without being asked, you MUST:",
    "- Track non-trivial tasks as Cairn work items (\`cairn work create/claim/close\`), not ad hoc todo lists.",
    "- Save durable decisions, bug fixes, and discoveries immediately with \`cairn memory save\`.",
    "- Search Cairn before starting work that may already exist (\`cairn search \\"<keywords>\\"\`).",
    "- Follow the Cairn session-close protocol before finishing.",
    "",
    "## Project (\`cairn status\`)",
    "\`\`\`json",
  ...
  if (ready) {
    lines.push(
      "",
      "## Ready work items (\`cairn work ready --explain\`)",
      "\`\`\`json",
      ready,
      "\`\`\`",
    );
  ...
  if (memory) {
    lines.push(
      "",
      "## Recent-session memory primer (\`cairn memory context\`)",
      "\`\`\`json",
      memory,
      "\`\`\`",
    );
  ...
  hooks: {
    onSessionStart: async (input) => {
      try {
        ...
        const additionalContext = await buildContext(cwd);
...
export type SetupTargetResult = Readonly<{
  ...
  // Present only for targets that ship a lifecycle-hook extension (Copilot CLI).
  extensionFile?: SetupFileResult;
}>;
...
function writeExtensionFile(path: string): SetupFileResult {
  const exists = existsSync(path);
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, copilotExtensionContent(), "utf8");
  return { action: exists ? "updated" : "created", path };
}
...
function applySetupTarget(target: SetupTarget, home: string): SetupTargetResult {
  ...
  return {
    ...
    target,
    ...(config.extensionFilePath === undefined
      ? {}
      : { extensionFile: writeExtensionFile(config.extensionFilePath(home)) }),
  };
}
...
function printSetupResult(result: SetupResult, json: boolean): void {
  ...
  for (const target of result.targets) {
    ...
    if (target.extensionFile !== undefined) {
      console.log(
        `  extension:   ${target.extensionFile.action} ${target.extensionFile.path}`,
      );
    }
  }
}
