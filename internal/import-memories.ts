...
// Internal one-off tool: imports observations from an external memory tool's
// JSON export into Cairn memories. Each observation becomes one memory;
// session and prompt records from the export are not imported (Cairn has no
// equivalent concept for either).
//
// Usage:
//   bun run internal/import-memories.ts <export.json> --path <dir> [--data-dir <dir>] [--project <name>] [--dry-run] [--json]
//
// Idempotent: every imported memory is saved with topic
// `import/memory/<sync_id>`, so re-running the script upserts the same
...
type ExternalObservation = Readonly<{
  id: number;
  sync_id: string;
  session_id: string;
  type: string;
...
function parseArguments(argv: readonly string[]): Options {
  ...
  if (!file) {
    console.error(
      "Usage: bun run internal/import-memories.ts <export.json> --path <dir> [--data-dir <dir>] [--project <name>] [--dry-run] [--json]",
    );
    process.exit(2);
  }
...
async function importObservation(
  ...
  const topic = `import/memory/${observation.sync_id}`;
  if (options.dryRun) {
...
async function main(): Promise<void> {
  ...
  const parsed = JSON.parse(readFileSync(options.file, "utf8")) as ExternalExport;
